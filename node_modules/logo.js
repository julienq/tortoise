...
exports.parser =
{
  ...
  parse_tokens: function(tokens)
  {
    ...
    {
      flexo.log("expression({0})".fmt(rbp));
      var t = token;
      advance();
      var left = t.nud();
      while (rbp < token.lbp) {
        flexo.log("  got lbp = {0}, keep going".fmt(token.lbp));
        t = token;
        advance();
        left = t.led(left);
      }
    ...
    function infix(id, bp, name)
    {
      var s = symbol(id, bp);
      s.led = function(left) { return [name, left, expression(bp)]; };
      return s;
    }
    ...
    prefix("(", function() {
        ...
        while (true) {
          ...
          group.push(expression(0));
        }
        return group;
      });

    function add_function(name, n)
    {
      var f = symbol(name);
      f.nud = function()
      {
        var node = [name];
        for (var i = 0; i < n; ++i) node.push(expression(70));
        return node;
      }
      return f;
    }
    ...
    // Make a new command with the expected number of arguments
    function add_command(name, n)
    {
      var c = symbol(name);
      c.cmd = function()
      {
        var node = [name];
        for (var i = 0; i < n; ++i) node.push(expression(0));
        return node;
      };
      return c;
    }
