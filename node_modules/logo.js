...
exports.interpreter =
{
  init: function()
  {
    ...
    this.add_primitive("WORD", 2, function() {
    ...
    this.add_primitive("FPUT", 2, function(thing, list) {
        thing = eval_(thing);
        list = eval_(list);
        if (list.listp) {
    ...
    this.add_primitive("LPUT", 2, function(thing, list) {
        thing = eval_(thing);
        list = eval_(list);
        if (list.listp) {
    ...
    this.add_primitive("FIRST", 1, function(thing) {
        thing = eval_(thing);
        if (thing.listp) {
    ...
    this.add_primitive("FIRSTS", 1, function(list) {
        return eval_(list).map(that.functions.FIRST.f);
      });

    this.add_primitive("LAST", 1, function(wordorlist) {
    ...
    this.add_primitive("BUTFIRST", 1, function(wordorlist) {
        var v = eval_(wordorlist);
        return v.listp ? v.butfirst() : v.toString().substr(1);
      });
    this.add_primitive("BF", 1, this.functions.BUTFIRST.f);

    this.add_primitive("BUTFIRSTS", 1, function(list) {
        return eval_(list).map(that.functions.BUTFIRST.f);
      });
    this.add_primitive("BFS", 1, this.functions.BUTFIRSTS.f);

    this.add_primitive("BUTLAST", 1, function(wordorlist) {
        var v = eval_(wordorlist);
        if (v.listp) {
          return v.butlast();
        } else {
    ...
    this.add_primitive("BL", 1, this.functions.BUTLAST.f);

    this.add_primitive("ITEM", 2, function(index, thing) {
        index = eval_(index);
        thing = eval_(thing);
        if (thing.listp) {
    ...
    this.add_primitive("COUNT", 1, function(thing) {
        return eval_(thing).length;
      });

    this.add_primitive("PRINT", 1, function() {
    ...
    // TODO fail on non-number values
    this.add_primitive("SUM", 2, function() {
        return flexo.foldr(function(x, y) { return x + that.eval_expr(y); }, 0,
          arguments);
      });

    this.add_primitive("PRODUCT", 2, function() {
        return flexo.foldr(function(x, y) { return x * that.eval_expr(y); }, 1,
          arguments);
      });

    this.add_primitive("BYE", 0, function() { this.bye(); });

    return this;
  },

  add_primitive: function(name, n_args, f)
  {
    this.parser.add_function(name, n_args);
    this.functions[name] = { primitive: true, f: f };
  },

  eval_tokens: function(tokens)
  {
    this.parser.parse_tokens(tokens).forEach((function(expr) {
        var v = this.eval_expr(expr);
        if (v !== undefined) {
  ...
  {
    if (typeof expr.arity === "string" && expr.arity === "call") {
      if (typeof this.functions[expr.value] === "object" &&
          typeof this.functions[expr.value].f === "function") {
        return this.functions[expr.value].f.apply(this, expr.args);
      } else {
        this.error("I don't know how to \"{0}\"".fmt(expr.value));
      }
    } else if (typeof expr.arity === "string" && expr.arity === "list") {
      return this.functions.LIST.f.apply(this, expr.value);
    } else {
...
exports.parser =
{
  ...
  advance: function(id)
  {
    ...
    if (this.i >= this.tokens.length) {
      ...
      console.log(">>> advance({0}): end of input = ".fmt(id || ""),
          this.token);
      return;
    }
